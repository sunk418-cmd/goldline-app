...
import { RiskAssessment, RiskTask, RiskFactor, INITIAL_RISK_DATA } from '../constants/riskData';
...
export function RiskAssessmentModal({ isOpen, onClose, onApply, appliedId }: RiskAssessmentModalProps) {
  ...
  const startNew = () => {
    ...
    setRiskFactors([{ id: (Date.now() + 1).toString(), jobType: '', factor: '', measure: '' }]);
    setIsEditing(true);
    setSearchTerm('');
  };
  ...
  const addFactor = () => {
    setRiskFactors([...riskFactors, { id: Date.now().toString(), jobType: '', factor: '', measure: '' }]);
  };
                        ...
                        <tr key={task.id} className="group">
                          ...
                          <td className="px-3 py-3">
                            <textarea 
                              ...
                              onInput={autoResize}
                              className="w-full bg-transparent border-none focus:ring-0 focus:outline-none p-0 min-h-[40px] resize-none overflow-hidden"
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-xs font-bold text-slate-400 w-1/3">위험요인</th>
                        <th className="px-3 py-2 text-left text-xs font-bold text-slate-400">사고예방대책</th>
                        <th className="px-3 py-2 w-10"></th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        ...
                        <tr key={factor.id} className="group">
                          ...
                          <td className="px-3 py-3">
                            <textarea 
                              ...
                              onChange={(e) => {
                                const newFactors = [...riskFactors];
                                newFactors[index].measure = e.target.value;
                                setRiskFactors(newFactors);
                              }}
                              onInput={autoResize}
                              className="w-full bg-transparent border-none focus:ring-0 focus:outline-none p-0 min-h-[40px] resize-none overflow-hidden"
                              placeholder="사고예방대책 입력"
                              rows={1}
                            />
                          </td>
                          <td className="px-3 py-3">
